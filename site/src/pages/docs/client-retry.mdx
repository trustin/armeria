...
You can customize the `strategy` by implementing <type://RetryStrategy>.
...
This will retry when one of <type://ResponseTimeoutException> and <type://UnprocessedRequestException> is raised or
the response's status is `409 Conflict`.

<Warning>

We declare a <type://Backoff> as a member and reuse it when a `strategy` returns it, so that we do not
...
the number of attempts made so far, and resets it to 0 when the <type://Backoff> returned by the strategy
...
<type://RetryStrategyWithContent> and specify it when you create an <type://WebClient>
...
If you create a <type://Backoff> using `.withMaxAttempts(maxAttempts)` in a <type://RetryStrategy>,
the <type://RetryingClient> which uses the <type://RetryStrategy> will stop retrying when the number of
...
threshold, it raises a <type://FailFastException>. When using both clients, you need to write a custom
<type://RetryStrategy> to handle this exception so that the <type://RetryingClient> does not attempt
