...
[SSOCircle](https://www.ssocircle.com/en/) in order to authenticate an end user. The following commands
...
Finally, you need to create your <type://SamlServiceProvider> with a <type://SamlServiceProviderBuilder>, and
attach it to your <type://Server>.
...
`armeria-saml` provides <type://SamlSingleSignOnHandler> to handle the response from an identity provider.
...
The following example shows a simple implementation of the <type://SamlSingleSignOnHandler>. In this example,
...
<type://Authorizer> can identify an authenticated session with a `Cookie` header in the following requests,
...
that you write your own <type://SamlSingleSignOnHandler> according to your authentication system.
